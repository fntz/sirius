...
class Sirius.LengthValidator extends Sirius.Validator
  ...
  validate: (value, attributes) ->
    @logger.info("LengthValidator: start validate '#{value}'", @logger.validator)
    if value?
...
class Sirius.ExclusionValidator extends Sirius.Validator
  ...
  # @return [Boolean]
  validate: (value, attributes) ->
    @logger.info("ExclusionValidator: start validate '#{value}'", @logger.validator)
...
class Sirius.InclusionValidator extends Sirius.Validator
  ...
  # @return [Boolean]
  validate: (value, attributes) ->
    @logger.info("InclusionValidator: start validate '#{value}'", @logger.validator)
    range = attributes['within'] || []
    if range.indexOf(value) > -1
      true
    else
...
class Sirius.FormatValidator extends Sirius.Validator
  ...
  # @return [Boolean]
  validate: (value, attributes) ->
    @logger.info("FormatValidator: start validate '#{value}'", @logger.validator)
...
class Sirius.NumericalityValidator extends Sirius.Validator
  ...
  # @return [Boolean]
  validate: (value, attributes = {}) ->
    @logger.info("NumericalityValidator: start validate '#{value}'", @logger.validator)
...
class Sirius.PresenceValidator extends Sirius.Validator
  ...
  validate: (value, attributes = true) ->
    @logger.info("PresenceValidator: start validate '#{value}'", @logger.validator)
    if value
      true
    else
      @msg = "Value required"
